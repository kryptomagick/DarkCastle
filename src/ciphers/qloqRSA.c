...
void qloq_init(struct qloq_ctx *ctx) {
    ctx->sk = BN_new();
    ctx->pk = BN_new();
    ctx->n = BN_new();
    ctx->M = BN_new();
...
void qloq_generate_keys(struct qloq_ctx *ctx, char * prefix, int psize) {
    BN_CTX * kgc = BN_CTX_new();
    //BN_CTX_start(kgctx);
    int randstat = 0;
    ...
    BN_one(z1);
    BN_zero(z0);
    unsigned char *seed[85760];
    FILE *randfile;
    /* Generate primes, let them not be equal */
    randfile = fopen("/dev/urandom", "rb");
    while (randstat != 1) {
        fread(seed, 1, 85760, randfile);

        RAND_seed(seed, 85760);
    ...
    BN_mul(ctx->n, p, q, kgc);
    /* Generate the mask */
    BN_mul(ctx->M, a, b, kgc);
    /* Build the totient */
    BN_sub(tmp0, p, z1);
    BN_sub(tmp1, q, z1);
    BN_mul(s, tmp0, tmp1, kgc);
    BN_sub(tmp0, a, z1);
    BN_sub(tmp1, b, z1);
    BN_mul(tmp2, tmp0 , tmp1, kgc);
    BN_mul(t, s , tmp2, kgc);
    /* Generate the public key */
    BN_rand_range(ctx->pk, t);
    BN_gcd(tmp0, ctx->pk, t, kgc);
    while (BN_cmp(tmp0, z1) != 0) {
        BN_rand_range(ctx->pk, t);
        BN_gcd(tmp0, ctx->pk, t, kgc);
    }
    /* Generate the private key */
    BN_mod_inverse(ctx->sk, ctx->pk, t, kgc);
    //BN_set_word(tmp0, 65);
    pkg_pk(ctx, prefix);
    //BN_CTX_end(kgctx);
    //BN_CTX_free(kgc);
